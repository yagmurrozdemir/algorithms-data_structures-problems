...
Queue <int> SieveOfEratosthenes(int n) {
    ...
    Queue <int> numbers;
    Queue <int> numbersToN;
    int myPrime, tester;
    for (int i=2 ;i<=n ;i++) numbersToN.enqueue(i);
    
    while (!numbersToN.isEmpty())
    {
        myPrime = numbersToN.dequeue();
        numbersToN.enqueue(myPrime);
        
        while ( (tester = numbersToN.dequeue()) != myPrime )
        {
            if (tester % myPrime) numbersToN.enqueue(tester); 
        }
        numbers.enqueue(myPrime);
    }
